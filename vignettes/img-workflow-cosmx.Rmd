...
We then use the `readCosmxSXE` function from the `SpatialExperimentIO` package to read the data into a `SpatialExperiment` object. Finally, we prepare the data for analysis with `SpaceTrooper` by updating the object and reading the cell polygons.
...
You can access additional metadata of the SpatialExperiment object `cos` as follows:
...
6. plot the centroids of the cell using `plotCentroids(cos)`. Colouring them for a specific cell metadata.
  ...
  gives a thorough overview of quality control steps for imaging-based spatial transcriptomics data, including CosMX-specific metrics. Here, we will simply compute `r BiocStyle::Biocpkg("SpaceTrooper")`'s aggregated QC score and keep the cells that pass the threshold.
...
```{r}
SpaceTrooper::plotMetricHist(spe, "QC_score")
```
## Processing
